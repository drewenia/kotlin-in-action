...
fun alphabet541v2() : String {
    val stringBuilder = StringBuilder()
    // Method’ları call ettiğin receiver value’yu belirt.
    return with(stringBuilder) {
        for (letter in 'A'..'Z'){
            // stringBuilder senin this’in olur …
            this.append(letter)
        }
...
function olduğunu unutma. img_5.png'de ki analogy uygulanabilir: Bir lambda, normal bir function’a benzer şekilde
behavior tanımlamanın bir yoludur. Receiver’lı bir lambda ise, extension function’a benzer şekilde behavior tanımlamanın
bir yoludur. İlk alphabet function’ı daha da refactor edelim ve ekstra stringBuilder variable’ından kurtulalım. */

fun alphabet541v4() = with(StringBuilder()){
    for (letter in 'A'..'Z'){
        append(letter)
    }
    append("\nNow I know the alphabet!")
...
fun main() {
    println(alphabet541())
    println(alphabet541v2())
    println(alphabet541v3())
    println(alphabet541v4())
}