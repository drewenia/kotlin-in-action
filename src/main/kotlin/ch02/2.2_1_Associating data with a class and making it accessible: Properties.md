# Associating data with a class and making it accessible: Properties
...
bir accessor declare edebilirsiniz. Person sınıfının concise declaration’ı, orijinal Java koduyla aynı underlying
...
(Demo java kodu) Kotlin sınıfı Person’ın Java kodundan nasıl kullanılacağını görebilirsiniz; Ocean adında yeni bir
Person object oluşturuluyor, kendisi bir student.

```java
public class Demo221 {
    public static void main(String[] args) {
        Person person = new Person("Ocean", true);
...
Kotlin’de tanımlanan Person, Java’dan kullanılırken Java’daki ile tamamen aynı şekilde görünür. Kotlin’in name
property’si, Java tarafında getName adlı bir getter method’u olarak ortaya çıkar.

```kotlin
fun main() {
    val person = Person("Bob", true)
    println("Person name : ${person.name} is student? : ${person.isStudent}")
