...
/* Hepimiz biliyoruz ki, Java bir object-oriented language olduğu için, tüm code’un class’ların method’u olarak
yazılmasını gerektirir. Genellikle bu güzel işler, ancak gerçekte neredeyse her büyük project, açıkça herhangi bir
single class’a ait olmayan çok fazla code ile sonuçlanır. Bazen bir operation, eşit derecede önemli bir rol oynayan iki
farklı class’ın object’leriyle çalışır. Diğer zamanlarda, bir primary object vardır, ancak operation’ı instance method
olarak ekleyerek onun API’sini şişirmek istemezsin. Sonuç olarak, hiçbir state veya instance method içermeyen
class’larla karşılaşırsın. Bu tür class’lar yalnızca birkaç static method için container olarak davranır. Bunun mükemmel
bir örneği, JDK’daki Collections class’ıdır. Kendi code’unda başka örnekler bulmak için, adı içinde Util geçen
class’lara bakabilirsin. Kotlin’de, bu anlamsız class’ları oluşturman gerekmez. Bunun yerine, function’ları doğrudan bir
source file’ın top level’ına, herhangi bir class dışında yerleştirebilirsin. Bu tür function’lar hâlâ file’ın en üstünde
declare edilen package’in member’larıdır ve başka package’lardan çağırmak istiyorsan hâlâ import etmen gerekir, ancak
gereksiz ekstra nesting seviyesi artık yoktur. joinToString function’ını doğrudan strings package’ına yerleştirelim.
**join.kt** adında bir file oluştur ve aşağıdaki içeriği ekle (bu seçim keyfi; başka bir filename de aynı şekilde
çalışır). */
...
fun performOperation323(){
    opCount++
}

fun reportOperationCount323(){
